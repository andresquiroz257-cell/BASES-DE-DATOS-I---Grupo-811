...
CREATE TABLE atencion (
    ...
    CONSTRAINT fk_atencion_medico_paciente FOREIGN KEY (medico_paciente_id) 
        REFERENCES medico_paciente(medico_paciente_id) ON DELETE CASCADE
);
CREATE INDEX idx_atencion_medico_paciente ON atencion(medico_paciente_id);
CREATE INDEX idx_atencion_fecha_hora ON atencion(fecha_hora);

--17  diagnostico_paciente 
CREATE TABLE diagnostico_paciente (
    ...
    CONSTRAINT fk_diagnostico_paciente_atencion FOREIGN KEY (atencion_id) 
        REFERENCES atencion(atencion_id) ON DELETE CASCADE,
    CONSTRAINT fk_diagnostico_paciente_diagnostico FOREIGN KEY (diag_id) 
        REFERENCES diagnostico(diag_id) ON DELETE RESTRICT

);
CREATE INDEX idx_diagnostico_paciente_atencion ON diagnostico_paciente(atencion_id);

--18  diagnostico_medico 
